...
export class JenisizinService {
  private readonly tableName = 'jenisizin';
  constructor(
  ...
  async create(data: any, trx: any, modifiedby: any) {
    try {
      ...
      throw new Error(`Error creating jenis izin: ${error.message}`);
    }
  }

  async findAllByIds(ids: { id: number }[]) {
    try {
      return 'This action read find All By IDS';
    } catch (error) {
  ...
  async update(id: number, data: any, trx: any) {
    try {
      ...
      console.error('Error updating jenis izin:', error);
      throw new Error('Failed to update jenis izin');
    }
  }
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN JENIS IZIN';
