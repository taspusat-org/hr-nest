...
export class ShiftService {
  private readonly tableName: string = 'shift';
  constructor(
  ...
  async create(data: any, trx: any) {
    try {
      ...
      const query = trx(`${this.tableName} as s`)
        .select([
          's.id as id',
          's.nama',
          's.statusaktif',
          ...
          's.modifiedby',
          dbMssql.raw(
            "ISNULL(FORMAT(s.created_at, 'dd-MM-yyyy HH:mm:ss'), ' ') as created_at",
          ),
          dbMssql.raw(
            "ISNULL(FORMAT(s.updated_at, 'dd-MM-yyyy HH:mm:ss'), ' ') as updated_at",
          ),
          'p.memo',
          'p.text as statusaktif_text',
        ])
        .leftJoin('parameter as p', 's.statusaktif', 'p.id')
        .orderBy(sortBy ? `s.${sortBy}` : 's.id', sortDirection || 'desc')
        .where('s.id', '<=', newItem.id); // Filter berdasarkan ID yang lebih kecil atau sama dengan newItem.id

      if (search) {
        query.where((builder) => {
          builder
            .orWhere('s.nama', 'like', `%${search}%`)
            .orWhere('s.keterangan', 'like', `%${search}%`)
            .orWhere('s.modifiedby', 'like', `%${search}%`)
            .orWhereRaw(
              "ISNULL(FORMAT(s.created_at, 'dd-MM-yyyy HH:mm:ss'), ' ') LIKE ?",
              [`%${search}%`],
            )
            .orWhereRaw(
              "ISNULL(FORMAT(s.updated_at, 'dd-MM-yyyy HH:mm:ss'), ' ') LIKE ?",
              [`%${search}%`],
            );
        });
      }
        ...
        for (const [key, value] of Object.entries(filters)) {
          if (value) {
            if (key === 'created_at' || key === 'updated_at') {
              query.andWhereRaw(
                "ISNULL(FORMAT(s.??, 'dd-MM-yyyy HH:mm:ss'), ' ') LIKE ?",
                [key, `%${value}%`],
              );
            } else if (key === 'text' || key === 'memo') {
              query.andWhere(`p.${key}`, '=', value);
            } else {
              query.andWhere(`s.${key}`, 'like', `%${value}%`);
            }
          }
        }
      }
  ...
  async findAll({ search, filters, pagination, sort }: FindAllParams) {
    try {
      ...
      const query = dbMssql(`${this.tableName} as s`)
        .select([
          's.id as id',
          's.nama',
          's.statusaktif',
  ...
  async findAllByIds(ids: { id: number }[]) {
    try {
      ...
      const query = dbMssql(`${this.tableName} as s`)
        ...
        .leftJoin('parameter as p', 's.statusaktif', 'p.id')
        .join(dbMssql.raw(`${tempData} as temp`), 's.id', 'temp.id')
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN SHIFT';
    ...
    // Mendefinisikan header kolom yang sesuai dengan findAll
    const headers = ['No.', 'NAMA', 'KETERANGAN'];
    ...
    data.forEach((row, rowIndex) => {
      ...
      worksheet.getCell(rowIndex + 6, 2).value = row.nama; // NAMA
      worksheet.getCell(rowIndex + 6, 3).value = row.keterangan; // KETERANGAN
    ...
    worksheet.getColumn(2).width = 25; // Lebar untuk nama
    worksheet.getColumn(3).width = 30; // Lebar untuk keterangan
  ...
  async update(id: number, data: any, trx: any) {
    try {
      ...
      const query = trx(`${this.tableName} as s`)
        ...
        .leftJoin('parameter as p', 's.statusaktif', 'p.id')
        .orderBy(sortBy ? `s.${sortBy}` : 's.id', sortDirection || 'desc')
        .where('s.id', '<=', Number(id)); // Filter berdasarkan ID yang lebih kecil atau sama dengan id
